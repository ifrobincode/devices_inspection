...
- 修改了文件夹格式：yyyy-mm-dd_hh_mm_ss
- 修改了用户输入info文件名的功能逻辑，现在支持：
  - 只输入文件名（不带扩展名）
  - 输入完整文件名（带扩展名）
  - 输入的文件名前、后带有误输入的空白字符也可以正确识别
  - 不输入，直接回车，使用默认文件名
...
- 修复了底层 paramiko 的异常输出信息影响 cmd 内显示信息的问题。

### 修改

- 修改了后一次运行脚本时，清理上一次运行时所采集的巡检信息和日志文件的逻辑。

### 删除

- 暂时删除了进入 Enable 模式的代码，不影响需要进入 Enable 模式的设备类型。
